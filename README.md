...
**Scheduled task** (`LyricFin: Get Timed Lyrics`) - fills tracks that are **missing** lyrics.
**Settings → Fetch all lyrics** - queues `LyricFin: Fetch All Lyrics` (force overwrite). Runs as a scheduled task so the browser request cannot time out.
...
**Skip (Instrumental) titles** (on by default): scheduled runs ignore `(Instrumental)` / `[Instrumental]`; force fetch **clears** lyrics on those tracks.
